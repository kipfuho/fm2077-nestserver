...
export class DatabaseService {
  ...
  */
  ...
  async findOneCategory(
    ...
    const cacheCategory: Category = await this.cacheManager.get(`category:${id}`);
    if(cacheCategory) {
      this.logger.log(`CACHE:::Find category, category:${id}`);
    ...
    await this.cacheManager.set(`category:${id}`, category, this.CACHE_TIME);
    this.logger.log("DB:::Find category");
    return category;
  }

  // Find all category
  async findCategory(): Promise<Category[] | null> {
    const categories = await this.categoryRepository.find();
    this.logger.log("DB:::Find all categories");
    return categories;
  }
  ...
  async findForumCategory(
    ...
    try {
      const cacheCategoryForum: Forum[] = await this.cacheManager.get(`category:${category_id}:forum`);
      if(cacheCategoryForum) {
        this.logger.log(`CACHE:::Find forums of a category succeeded, category:${category_id}`);
      ...
      await this.cacheManager.set(`category:${category_id}:forum`, forums, this.CACHE_TIME);
      this.logger.log(`DB:::Find forums of a category succeeded, category:${category_id}`);
      return forums;
    } catch(error) {
      this.logger.error(error);
      return null;
  ...
  */
  ...
  async createNewThread(
    ...
    try {
      ...
      await this.cacheManager.set(`forum:${forum_id}:thread:lastest`, thread);
  ...
  async updateMessageReaction(
    ...
    try {
      ...
      if(add) {
        ...
        if(reaction) {
          this.logger.log(`Add message reaction failed, exist reaction of user:${user_id} message:${message_id}`);
          return false;
        }

        messageData.message.reactions[reaction_type]++;
        await Promise.all([this.messageRepository.save(messageData.message), this.reactionRepository.save(newReaction)]);
        ...
        if(!reaction) {
          this.logger.log(`Remove message reaction failed, not exist reaction of user:${user_id} message:${message_id}`);
          return false;
        }

        messageData.message.reactions[reaction.type]--;
        await Promise.all([this.messageRepository.save(messageData.message), this.reactionRepository.delete({message_id, user_id})]);
        if(messageData.cache) {
          await this.cacheManager.set(`message:${message_id}`, messageData.message);
        }
      }

      this.logger.log(`Update message reaction succeeded, message:${message_id}`);
      return true;
    } catch(error) {
      this.logger.error(error);
      return false;
